...
@section('content')
        ...
        <div class="edit-admin">
            ...
            <form action="{{ route('admin.admins.update', $admin->id) }}" 
                        ...
                        <div class="form-card">
                            <div class="card-header">
                                ...
                                <div class="card-title">
                                    ...
                                    <p>تعديل البيانات الأساسية للمشرف</p>
                                </div>
                            </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="d-flex align-items-center h-100">
                                        <div class="d-flex align-items-center">
                                            <span class="toggle-label">{{ $admin->is_active ? 'نشط' : 'غير نشط' }}</span>
                        ...
                        <div class="form-card">
                            <div class="card-header">
                                ...
                                <div class="card-title">
                                    ...
                                    <p>اترك الحقول فارغة إذا لم ترد تغيير كلمة المرور</p>
                                </div>
                            </div>
                        ...
                        <div class="form-card">
                            ...
                            <div class="avatar-upload">
                                <div class="avatar-preview" id="avatarPreview">
                                    @if($admin->avatar)
                                        <img src="{{ $admin->avatar_url }}" alt="{{ $admin->name }}" id="avatarImage">
                                    @else
                                        <span id="avatarInitial">{{ substr($admin->name, 0, 1) }}</span>
                                    @endif
                                    <button type="button" class="avatar-remove" id="removeAvatar" onclick="removeAvatar()">
                        ...
                        <div class="form-card">
                            ...
                            <div class="row g-3">
                                @foreach($roles as $role)
                                    <div class="col-12">
                                        <label class="role-card @if(old('role', $admin->roles->first()->id ?? '') == $role->id) selected @endif">
                                            <input type="radio" 
                                                   name="role" 
                                                   value="{{ $role->id }}" 
                                                   class="role-radio"
                                                   {{ old('role', $admin->roles->first()->id ?? '') == $role->id ? 'checked' : '' }}
                                                   required>
                                            <div class="role-icon">
                                                <i class="fas 
                ...
                <div class="form-card">
                    <div class="card-header">
                        ...
                        <div class="card-title">
                            ...
                            <p>صلاحيات إضافية ممنوحة للمشرف</p>
                        </div>
                    </div>
                ...
                <div class="form-actions">
                    ...
                    @if(!$admin->isSuperAdmin() || auth()->id() == $admin->id)
    ...
    <div class="spinner-overlay" id="spinnerOverlay">
        <div class="spinner-container">
            ...
            <h6 class="mt-3">جاري حفظ التغييرات...</h6>
        </div>
    </div>
@endsection
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#avatarInput').change(function(e) {
                ...
                if (file) {
                    ...
                    reader.onload = function(e) {
                        ...
                        preview.empty();
                        preview.append(`<img src="${e.target.result}" alt="Avatar" id="avatarImage">`);
                        preview.append(`<button type="button" class="avatar-remove" onclick="removeAvatar()">
                                        <i class="fas fa-times"></i>
                                    </button>`);
                    }
            ...
            @if($admin->isSuperAdmin() && auth()->id() != $admin->id)
                Swal.fire({
                    icon: 'warning',
                    title: 'تنبيه',
                    text: 'لا يمكنك تعديل المشرف الرئيسي',
                    confirmButtonText: 'حسناً',
                    confirmButtonColor: '#696cff'
                }).then(() => {
        ...
        function removeAvatar() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    const preview = $('#avatarPreview');
                    preview.empty();
                    preview.append(`<span id="avatarInitial">{{ substr($admin->name, 0, 1) }}</span>`);
                    preview.append(`<button type="button" class="avatar-remove" onclick="removeAvatar()">
                                    <i class="fas fa-times"></i>
                                </button>`);
                    
                    // Add hidden input to remove avatar
                    if (!$('#remove_avatar').length) {
                        $('#editAdminForm').append('<input type="hidden" name="remove_avatar" id="remove_avatar" value="1">');
                    }
        ...
        function deleteAdmin(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ route("admin.admins.destroy", $admin->id) }}',
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
        ...
        $('#editAdminForm input, #editAdminForm select, #editAdminForm textarea').change(function() {
            formChanged = true;
        });
