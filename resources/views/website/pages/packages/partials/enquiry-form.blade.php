...
@endphp
<form class="{{ $isCruiseEnquiry ? 'nile-cruise-enquiry' : '' }}" method="post" action="{{ route('website.inquiries.store') }}">
    @csrf
    ...
    <div class="input-box">
        ...
        <div class="form-group">
            ...
            <input class="form-control" type="text" id="enquiry_name_{{ $suffix }}" name="name" required placeholder="{{ $isCruiseEnquiry ? __('Enter your full name') : __('Your name') }}"
                value="{{ old('name') }}">
        </div>
        @error('name')
    ...
    <div class="input-box">
        ...
        <div class="form-group">
            ...
            <input class="form-control" type="email" id="enquiry_email_{{ $suffix }}" name="email" required placeholder="{{ $isCruiseEnquiry ? 'your.email@example.com' : __('Email address') }}"
                value="{{ old('email') }}">
        </div>
        @error('email')
            <small class="text-danger d-block mt-1">{{ $message }}</small>
    ...
    <div class="input-box">
        ...
        <div class="form-group">
            <input class="form-control" type="tel" id="enquiry_phone_{{ $suffix }}" name="phone" placeholder="{{ $isCruiseEnquiry ? '+1 (555) 123-4567' : __('Phone Number') }}"
                value="{{ old('phone') }}">
        </div>
        @error('phone')
            <small class="text-danger d-block mt-1">{{ $message }}</small>
    ...
    <div class="input-box">
        ...
        <div class="form-group">
            ...
            <input id="enquiry_travel_date_{{ $suffix }}" name="travel_date" class="form-control" type="date" required value="{{ old('travel_date', $isCruiseEnquiry ? now()->toDateString() : null) }}">
        </div>
        @error('travel_date')
            <small class="text-danger d-block mt-1">{{ $message }}</small>
        @enderror
    ...
    <div class="input-box">
        ...
        <div class="form-group">
            ...
            <textarea class="message-control form-control" id="enquiry_comment_{{ $suffix }}" name="comment" placeholder="{{ __('Please advise your tour requirements') }}">{{ old('comment') }}</textarea>
        </div>
        @error('comment')
            <small class="text-danger d-block mt-1">{{ $message }}</small>
        @enderror
    ...
    <div class="btn-box">
        <button type="submit" class="submit-btn" style="width:100%">@if($isCruiseEnquiry)<i class="la la-paper-plane" aria-hidden="true"></i> @endif{{ __('Submit Enquiry') }}</button>
    </div>
...
@if($isCruiseEnquiry)
    @once
        <style>
            .nile-cruise-enquiry .label-text { font-size: 13px; font-weight: 600; }
            .nile-cruise-enquiry .form-icon { display: none; }
            .nile-cruise-enquiry .form-control { border: 1px solid #d8dfeb; border-radius: 12px; padding: 13px 14px; font-size: 14px; min-height: 47px; }
            .nile-cruise-enquiry select.form-control { appearance: auto; }
            .nile-cruise-enquiry .quantity-control { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px; padding: 11px 13px; border: 1px solid #f0e8df; border-radius: 12px; background: #fff; }
            .nile-cruise-enquiry .quantity-control label { margin: 0; color: #1c325c; font-size: 12px; font-weight: 600; }
            .nile-cruise-enquiry .qty-buttons { margin: 0; gap: 6px; }
            .nile-cruise-enquiry .qty-btn { width: 32px; height: 32px; background: #c5955b; color: #fff; }
            .nile-cruise-enquiry .qty-input { border: 0; background: transparent; padding: 0; width: 39px; color: #1c325c; font-weight: 700; appearance: textfield; -moz-appearance: textfield; }
            .nile-cruise-enquiry .qty-input::-webkit-inner-spin-button { -webkit-appearance: none; }
            .nile-cruise-enquiry .message-control { min-height: 110px; }
            .nile-cruise-enquiry .submit-btn { border: 0; border-radius: 13px; padding: 13px; background: linear-gradient(110deg, #c5955b, #b7880a); color: #1c325c; }
        </style>
    @endonce
@endif
