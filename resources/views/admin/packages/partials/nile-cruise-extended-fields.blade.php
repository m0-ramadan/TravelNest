            ...
            return [
                ...
                'itinerary' => $duration->itineraryDays->map(fn($day) => [
                    ...
                    'activities' => $day->activities->map(fn($activity) => [
                        'section_title' => $activity->display_section_title,
                        'section_description' => $activity->display_section_description,
                        ...
                        @foreach((array)($duration['itinerary']??[]) as $dayIndex => $day)
                            <div class="nc-repeat" data-nc-day data-day-index="{{ $dayIndex }}"><div class="nc-repeat-head"><strong>{{ __('Day') }} {{ $day['day_number'] ?? ($dayIndex+1) }}</strong><button type="button" class="btn btn-outline-danger btn-sm" data-nc-remove>{{ __('Delete') }}</button></div><div class="nc-grid">
                                ...
                                @foreach((array)($day['activities']??[]) as $a => $activity)
                                    <div class="nc-repeat" data-nc-activity><div class="nc-grid nc-grid-3"><div><label class="form-label">{{ __('Tour Section') }}</label><input class="form-control" name="nile_cruise[durations][{{ $d }}][itinerary][{{ $dayIndex }}][activities][{{ $a }}][section_title]" value="{{ $activity['section_title'] ?? '' }}" placeholder="Afternoon Tour"></div><div><label class="form-label">{{ __('Site Title') }}</label><input class="form-control" name="nile_cruise[durations][{{ $d }}][itinerary][{{ $dayIndex }}][activities][{{ $a }}][title]" value="{{ $activity['title'] ?? '' }}"></div><div><label class="form-label">{{ __('Link Attraction') }}</label><select class="form-select" name="nile_cruise[durations][{{ $d }}][itinerary][{{ $dayIndex }}][activities][{{ $a }}][attraction_id]"><option value="">-</option>@foreach($attractions ?? collect() as $att)<option value="{{ $att->id }}" {{ (int)($activity['attraction_id']??0)===(int)$att->id?'selected':'' }}>{{ adminTrans($att->name) }}</option>@endforeach</select></div><div class="nc-full"><label class="form-label">{{ __('Tour Section Description') }}</label><textarea class="form-control" rows="2" name="nile_cruise[durations][{{ $d }}][itinerary][{{ $dayIndex }}][activities][{{ $a }}][section_description]">{{ $activity['section_description'] ?? '' }}</textarea></div><div class="nc-full"><label class="form-label">{{ __('Site Details') }}</label><textarea class="form-control" rows="2" name="nile_cruise[durations][{{ $d }}][itinerary][{{ $dayIndex }}][activities][{{ $a }}][description]" placeholder="{{ __('Details') }}">{{ $activity['description'] ?? '' }}</textarea></div><div><button type="button" class="btn btn-outline-danger" data-nc-remove>{{ __('Delete') }}</button></div></div></div>
                                @endforeach
                            </div></div></div>
                        @endforeach
                    </div></div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function activityTemplate(d,day,a){return `<div class="nc-repeat" data-nc-activity><div class="nc-grid nc-grid-3"><div><label class="form-label">{{ __('Tour Section') }}</label><input class="form-control" name="nile_cruise[durations][${d}][itinerary][${day}][activities][${a}][section_title]" placeholder="Afternoon Tour"></div><div><label class="form-label">{{ __('Site Title') }}</label><input class="form-control" name="nile_cruise[durations][${d}][itinerary][${day}][activities][${a}][title]"></div><div><label class="form-label">{{ __('Attraction') }}</label>${attractionSelect(`nile_cruise[durations][${d}][itinerary][${day}][activities][${a}][attraction_id]`)}</div><div class="nc-full"><label class="form-label">{{ __('Tour Section Description') }}</label><textarea class="form-control" rows="2" name="nile_cruise[durations][${d}][itinerary][${day}][activities][${a}][section_description]"></textarea></div><div class="nc-full"><label class="form-label">{{ __('Site Details') }}</label><textarea class="form-control" rows="2" name="nile_cruise[durations][${d}][itinerary][${day}][activities][${a}][description]"></textarea></div><div><button type="button" class="btn btn-outline-danger" data-nc-remove>{{ __('Delete') }}</button></div></div></div>`;}
