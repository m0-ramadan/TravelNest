...
@section('css')
    ...
    <style>
        ...
        .order-item.active-user {
            border-color: var(--success-color);
        }

        .order-item.inactive-user {
...
@section('content')
    @php
        $stats = [
            ...
            'new_this_month' => \App\Models\User::whereBetween('created_at', [
                now()->startOfMonth(),
                now()->endOfMonth(),
            ])->count(),
        ];
                    ...
                    <div class="card-body p-4">
                        ...
                        @else
                            @foreach ($users as $user)
                                @php
                                    $isActive = $user->is_active ?? true ? 'active' : 'inactive';
                                    $isVerified = !empty($user->email_verified_at);
                                @endphp
                                <div class="order-item {{ $isActive == 'active' ? 'active-user' : 'inactive-user' }}">
                                    <div class="order-header-info">
                                        ...
                                        <div class="order-date">
                                            ...
                                            {{ optional($user->created_at)->translatedFormat('d M Y - h:i A') }}
                                        </div>
                                    </div>
...
@section('js')
    ...
    <script>
        ...
        function filterByVerified(value) {
            updateUrl({
                verified: value,
                status: null
            });
